...
function rcb_render_component_builder_block( $attributes, $content ) {
	...
	$template_type = strtolower( get_post_meta( $template_id, '_component_type', true ) );
...
function rcb_render_query_type( $attributes, $content, $template_id, $unique_id, $global_post ) {
	...
	if ( wp_doing_ajax() && isset( $_POST['paged'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- paged is an integer, sanitized immediately below.
		$paged = intval( $_POST['paged'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing -- paged is integer-only; nonce is handled by the AJAX handler that calls this renderer.
	} else {
		$paged_qv = get_query_var( 'paged' );
		$page_qv  = get_query_var( 'page' );
		$paged    = $paged_qv ? $paged_qv : ( $page_qv ? $page_qv : 1 );
	}
			...
			if ( ( empty( $data['text'] ) || empty( $data['url'] ) ) && ! empty( $dynamic_source ) ) {
				...
				if ( ( $lower_source === 'permalink' || $lower_source === 'post_url' || strpos( $lower_source, 'url' ) !== false ) && $post ) {
					...
					if ( ! empty( $resolved_meta ) ) {
						if ( empty( $data['text'] ) ) {
							$data['text'] = $resolved_meta;
						}
						if ( empty( $data['url'] ) ) {
							$data['url'] = $resolved_meta;
						}
					}
				}
			}
...
function rcb_render_visual_nodes_with_visibility( $nodes, $content_data, $styles_data, $mode, $post = null, $visibility = array(), $inner_blocks_content = '', &$style_registry = '', $instance_id = '' ) {
	...
	foreach ( $nodes as $node ) {
		...
		if ( ! empty( $visibility ) ) {
			if ( $type === 'heading' && isset( $visibility['showTitle'] ) && ! $visibility['showTitle'] && $dynamic_source === 'post_title' ) {
				continue;
			}
			if ( $type === 'text' && isset( $visibility['showExcerpt'] ) && ! $visibility['showExcerpt'] && ( $dynamic_source === 'post_excerpt' || $dynamic_source === 'post_content' ) ) {
				continue;
			}
			if ( $type === 'image' && isset( $visibility['showImage'] ) && ! $visibility['showImage'] && $dynamic_source === 'featured_image' ) {
				continue;
			}
			if ( $type === 'button' && isset( $visibility['showButton'] ) && ! $visibility['showButton'] ) {
				continue;
			}
		}
		...
		if ( $type === 'button' ) {
			...
			if ( ! isset( $raw_styles['display'] ) ) {
				$raw_styles['display'] = 'inline-flex';
			}
			if ( ! isset( $raw_styles['alignItems'] ) ) {
				$raw_styles['alignItems'] = 'center';
			}
			if ( ! isset( $raw_styles['justifyContent'] ) ) {
				$raw_styles['justifyContent'] = 'center';
			}
			if ( ! isset( $raw_styles['gap'] ) ) {
				$raw_styles['gap'] = '8px';
			}
			if ( ! isset( $raw_styles['transition'] ) ) {
				$raw_styles['transition'] = 'all 0.3s ease-in-out';
			}
			if ( ! isset( $raw_styles['textDecoration'] ) ) {
				$raw_styles['textDecoration'] = 'none';
			}
		...
		if ( $type === 'button' ) {
			...
			// Icon Styles & Hover
			$icon_init_css = '';
			if ( ! empty( $raw_styles['iconColor'] ) ) {
				$icon_init_css .= "color: {$raw_styles['iconColor']} !important;";
			}
			if ( ! empty( $raw_styles['iconBgColor'] ) ) {
			...
			if ( ! empty( $ih_bg ) ) {
				$icon_hover_css .= "background-color: {$ih_bg} !important;";
			}

			$ih_color = $raw_styles['iconHoverColor'] ?? '';
			if ( ! empty( $ih_color ) ) {
				$icon_hover_css .= "color: {$ih_color} !important;";
			}
...
function rcb_build_inline_style( $data, $node_type = '' ) {
	...
	foreach ( $data as $prop => $val ) {
		...
		$suffix = '';
		if ( in_array( $kebab, array( 'font-size', 'margin', 'padding', 'gap', 'border-width', 'letter-spacing', 'min-height', 'min-width', 'height', 'width', 'flex-basis' ), true ) && is_numeric( $clean_val ) ) {
			$suffix = 'px';
		}
...
function rcb_generate_responsive_css( $selector, $styles ) {
	...
	foreach ( $styles as $prop => $data ) {
		...
		foreach ( $values as $device => $val ) {
			...
			$prop_name = $kebab;
			if ( $prop_name === 'display-mode' ) {
				$prop_name = 'display';
			}
			if ( $prop_name === 'flex-gap' || $prop_name === 'grid-gap' ) {
				$prop_name = 'gap';
			}
			...
			$suffix = '';
			if ( in_array( $prop_name, array( 'font-size', 'margin', 'padding', 'gap', 'border-width', 'letter-spacing', 'min-height', 'min-width', 'height', 'width', 'flex-basis' ), true ) && is_numeric( $clean_val ) ) {
				$suffix = 'px';
			}
			...
			if ( $device === 'desktop' ) {
				$desktop .= "{$prop_name}:{$clean_val}{$suffix} !important;";
			}
			if ( $device === 'tablet' ) {
				$tablet .= "{$prop_name}:{$clean_val}{$suffix} !important;";
			}
			if ( $device === 'mobile' ) {
				$mobile .= "{$prop_name}:{$clean_val}{$suffix} !important;";
			}
		}
	}
	...
	$out = '';
	if ( $desktop ) {
		$out .= "{$selector} { {$desktop} }\n";
	}
	if ( $tablet ) {
		$out .= "@media (max-width: 1024px) { {$selector} { {$tablet} } }\n";
	}
	if ( $mobile ) {
		$out .= "@media (max-width: 767px) { {$selector} { {$mobile} } }\n";
	}
